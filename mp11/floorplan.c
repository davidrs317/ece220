...
/*This MP had a lot of parts so I'm gonna break this down with little bullet points
-is_leaf_node was a simple function that just required me to check whether or not the node had children,
  if it had no children I returned 1, if it had children I returned 0
-is_internal_node was another simple function. I had to check whether or not the node had children,
  if it had children I returned 1, if it did not I returned 0
-is_in_subtree was slightly more complex than the previous two functions but not much, 
  for this one I had to check if the two pointers that were passed in pointed to the same node
  if they did I returned 1, then I had to check if the second pointer that was passed in pointed
  to one of the first nodes children, if it did I returned 1. Then I recursed up the tree, checking if
  the second node was a child of one of the first nodes children, which would return 1 if it was. If all those failed
  0 was returned.
-rotate was a simple function that just required me to make a temp variables to store width and height, and use those variables
  to swap the width of the height of the node that was passed in
-recut was a simple function that tested if a node was internal, from there I had to check the value of the cutline and switch it to the 
  opposite value
-swap module required some dynamic memory allocation but was still simple. I allocated two temporary nodes to hold the modules pointers of the nodes
  that were passed in. I then used those temp values to swap the modules of the nodes passed in and then removed the temporary nodes from the heap
-swap_topology was slightly more complicated. To solve it I had to test whether a node was its parent's left or right child. From there I used a temporary node
  I had allocated at the beginning to store the value of one node's parent as it would be necessary to swap the parent's. Then, depending on whether or not node a
  was the left or right child. I swapped node a's parent's left or right with node b's parent's left or right and then swapped node a's and node b's parent nodes
  lastly I removed the temporary node from the heap
-postfix_traversal wasn't too bad since the algorithm was given. For this one I followed the algorithm, going over the right and left subtrees recursively
  after that I assigned the expression unit within the array at the location defined by the nth pointer with the module and cutline of the node pointed to 
  by the pointer that was passed in.
-init_slicing_tree was probably the most difficult function of the MP, but I followed the outline given in the lab which helped. To start I allocated space for the new node
  and set its parent equal to the parent node that was passed in to the function. From there I tested to see if was the last node to create by checking if the counter passed in
  was equal to the amount of modules that existed as defined in the global variables and assigned its module pointer to the address of modules[n] with all other values being NULL. 
  If that failed, the node was an internal node so its cutline was set to V by definition of the slicing tree this function was supposed to generate. From there, I allocated memory
  for a node that would be the right child of the node I created earlier, this node by nature of the tree is automatically a module so I set it's module pointer to the address of 
  modules[n], its parent as ptr, and all other values to NULL. Then I created a left pointer that was generated by a recursive call of the function that passed in the new node pointed
  at the beginning and n+1, returning pointer.
...
int is_leaf_node(node_t* ptr) {
  ...
  //check for for children, return 1 if none
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  //check for children. if children exist, return 1 
  if((ptr->left != NULL) || (ptr->right != NULL)){ 
    return 1;
  }
  return 0;
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  //Base case 1, if either pointer is null return 0 as they aren't in the tree
  if(a == NULL || b == NULL){
    return 0;
  }
  //Base case 2, if the pointers point to the same node, return 1 as that counts as being in the subtree
  if(a == b){
    return 1;
  }
  //Base case 3, if b is one of the children of a, return 1 as that means it would be in one of a's subtrees
  if(a->left == b || a->right == b){
    return 1;
  }
  //Recursive case, tests b's parent to see if that is one of a's children meaning that b is in the subtree
  return is_in_subtree(a, b->parent);
  /*int left = is_in_subtree(a->left, b);
  if(left == 1){
    return 1;
  }*/
 return 0;
...
void rotate(node_t* ptr) {
  // TODO:
  int width, height;
  //declare temporary values to do the swap
  width = ptr->module->w;
  height = ptr->module->h;
...
void recut(node_t* ptr) {
  ...
  //check to see what the current cutline is, flip the value to the opposite of that
  if(ptr->cutline == H){
    ptr->cutline = V;
  }else{
    ptr->cutline = H;
...
void swap_module(node_t* a, node_t* b) {
  ...
  //declare temporary node_t pointers to hold the original modules for swapping
  node_t* a_mod = (node_t*)malloc(sizeof(node_t));
  node_t* b_mod = (node_t*)malloc(sizeof(node_t));
  //swap the modules
  a_mod->module = a->module;
  b_mod->module = b->module;
  a->module = b_mod->module;
  b->module = a_mod->module;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  //create a temporary node_t pointer to hold the parent of one of the nodes for swapping
  node_t* temp = (node_t*)malloc(sizeof(node_t));
  //test to see if the node is the left node  
  if(a == a->parent->left){
    //store a's parent for swapping
    temp->parent = a->parent;
    //change the original parent children values to their swapped values
    a->parent->left = b;
    b->parent->right = a;
    //swap the parents
    a->parent = b->parent;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  //Base case, cannot traverse a tree that has no values
  if(ptr == NULL) return;
  //traverse the left subtree recursively calling the Postfix function
  postfix_traversal(ptr->left, nth, expression);
  //traverse the right subtree by recursively calling the Postfix function
  ...
  else if(expression[*nth].cutline != UNDEFINED_CUTLINE){
    expression[*nth].module = NULL;
  }*/
  //assign the current node's module and cutline to the current expression_unit within the array defined by nth
  //have to dereference nth to update the expression at the address
  expression[*nth].module = ptr->module;
  expression[*nth].cutline = ptr->cutline;
  //
  //dereference nth and increment the address
  (*nth)++;
  // TODO:
}
...
int get_total_resource(node_t* ptr)
{
  ...
  //postorder traversal to get all the possible resources
  //store the resources from the left and right subtrees in their respective variables
  left_res = get_total_resource(ptr->left);
  right_res = get_total_resource(ptr->right);
  //sum up the left and right subtrees' resources
  tot_res = left_res + right_res;
  //visit root
  if(ptr->left == NULL && ptr->right == NULL){
    //complete getting the total resources by visiting the root and adding its resource to the total
    tot_res = tot_res + ptr->module->resource;
  }
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  ptr->parent = par; //set the new nodes parent node

  //Base case
  if(n == num_modules-1){
    //assign the data to the module pointed to by ptr
    //make sure all node values are assigned, since this is a child, everything but the module will be null
  ...
  //internal node
  //this tree will only have vertical cutlines by nature
  ptr->module = NULL;
  ptr->cutline = V;

  //right child
  //create a new node that will be the first new node's right child
  node_t* right = (node_t*)malloc(sizeof(node_t));
  //assign the value from the module array at n
  //ensure all node values are assigned to prevent segmentation fault
  //since all right nodes of this tree are leaf nodes by nature, all values but the module will be null
  ...
  //create new nodes down the left side of the tree and place the next module in the module array in the tree
